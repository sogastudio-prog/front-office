...
require_once plugin_dir_path(__FILE__) . 'includes/commercial/class-sdfo-commercial-admin.php';
SDFO_Commercial_CPTs::register();
SDFO_Commercial_Stripe_Sync::register();
SDFO_Commercial_Admin::register();
...
final class SD_Front_Office_Scaffold {
    ...
    /**
     * [sdfo_package_select] shortcode.
     *
     * Renders clickable package cards sourced from the sd_comm_package CPT.
     * On click, stores sd_package_key in sessionStorage and injects a hidden
     * field into any CF7 form on the same page. Integrates cleanly with the
     * existing wp_footer JS that reads sessionStorage on form submission.
     *
     * Usage: [sdfo_package_select]
     * Optional attributes:
     *   heading="Choose your plan"  — override the section heading
     *   show_all="1"               — include inactive/draft packages (admin preview)
     */
    public static function shortcode_package_select(array $atts = []): string {
        $atts = shortcode_atts([
            'heading'  => 'Choose your plan',
            'show_all' => '0',
        ], $atts, 'sdfo_package_select');
        ...
        // If no published packages yet, fall back to showing anything with a Stripe price.
        if (empty($packages)) {
            $all_posts = get_posts([
        ...
        ob_start();
        ?>
        <div class="sdfo-package-select" id="sdfo-package-select">
            <?php if (!empty($atts['heading'])) : ?>
                <h2 class="sdfo-package-select__heading"><?php echo esc_html($atts['heading']); ?></h2>
            <?php endif; ?>
        ...
        <style>
        .sdfo-package-select { margin: 0 0 32px; }
        .sdfo-package-select__heading { font-size: 22px; font-weight: 800; margin-bottom: 16px; color: #0f172a; }
        .sdfo-package-select__grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 14px; }
        .sdfo-package-select__note { margin-top: 14px; font-size: 13px; color: #64748b; }
        .sdfo-pkg-card {
        ...
        <script>
        (function() {
            var grid = document.getElementById('sdfo-package-select');
            if (!grid) return;

            var cards = grid.querySelectorAll('.sdfo-pkg-card');

            function selectCard(card) {
                cards.forEach(function(c) {
                    c.classList.remove('is-selected');
                    c.setAttribute('aria-pressed', 'false');
                    c.querySelector('.sdfo-pkg-card__cta').textContent = 'Select';
                });
                card.classList.add('is-selected');
                card.setAttribute('aria-pressed', 'true');
                card.querySelector('.sdfo-pkg-card__cta').textContent = 'Selected ✓';

                var key = card.getAttribute('data-package-key');
                try { sessionStorage.setItem('sd_package_key', key); } catch(e) {}

                document.querySelectorAll('form.wpcf7-form').forEach(function(form) {
                    var existing = form.querySelector('input[name="sd_package_key"]');
                    if (!existing) {
                        existing = document.createElement('input');
                        existing.type = 'hidden';
                        existing.name = 'sd_package_key';
            ...
            var storedKey = '';
            try { storedKey = sessionStorage.getItem('sd_package_key') || ''; } catch(e) {}

            cards.forEach(function(card) {
                if (storedKey !== '' && card.getAttribute('data-package-key') === storedKey) {
                    selectCard(card);
                }
            ...
            if (!grid.querySelector('.sdfo-pkg-card.is-selected') && cards.length > 0) {
                selectCard(cards[0]);
            }
        })();
        </script>
    ...
    private static function resolve_checkout_pricing_for_prospect(int $prospect_post_id): array {
        ...
        // ---------------------------------------------------------------
        // 1. CPT-primary resolution (front-office plane).
        //    Tries published first, then any status (draft packages with
        //    a synced Stripe price are valid during setup).
        // ---------------------------------------------------------------
        if (class_exists('SDFO_Commercial_CPTs', false)) {
            $cpt_result = self::resolve_pricing_via_cpt($package_key, $authorization_code, $prospect_post_id);
        ...
        // ---------------------------------------------------------------
        // 2. Kernel resolver fallback — only available when the kernel
        //    commercial module is loaded on this plane.
        // ---------------------------------------------------------------
        if (function_exists('sd_resolve_commercial_profile')) {
            ...
            return [
                'ok'                     => true,
                'profile_source'         => 'commercial_kernel',
        ...
        // ---------------------------------------------------------------
        // 3. Last-resort fallback: use a previously stored resolved price.
        //    Covers the case where checkout is retried after a first pass.
        // ---------------------------------------------------------------
        ...
        if ($fallback_price !== '') {
            ...
            return [
                ...
                'profile_id'     => (string) get_post_meta($prospect_post_id, 'sd_commercial_profile_key', true),
                'package_key'    => $package_key,
                'authorization_code' => $authorization_code,
                'stripe_price_id'=> $fallback_price,
                'plan_label'     => (string) get_post_meta($prospect_post_id, 'sd_resolved_plan_label', true),
                'discount_policy'        => [],
    ...
    /**
     * Resolve checkout pricing directly from SDFO_Commercial_CPTs.
     * This is the primary resolver on the front-office plane, which does
     * not have the kernel plugin loaded.
     *
     * Resolution order:
     *   a) Auth code → linked profile → package billing
     *   b) Package default profile → billing
     *   c) Package billing only (no profile)
     */
    private static function resolve_pricing_via_cpt(
        string $package_key,
        string $authorization_code,
        int $prospect_post_id
    ): array {
        // a) Try auth code first.
        if ($authorization_code !== '') {
            $validation = SDFO_Commercial_CPTs::validate_auth_code($authorization_code, $package_key);
            if (!empty($validation['valid']) && !empty($validation['profile'])) {
                $profile = $validation['profile'];
                $base_key = $profile['base_package_key'] ?: $package_key;
                $package  = SDFO_Commercial_CPTs::get_package_any_status($base_key)
                         ?? SDFO_Commercial_CPTs::get_package_any_status($package_key);

                if ($package && !empty($package['billing']['stripe_price_id'])) {
                    return self::build_cpt_pricing_result(
                        $package, $profile, $authorization_code, $prospect_post_id
                    );
                }
            }
        }

        // b) Try published package + its default profile.
        $package = SDFO_Commercial_CPTs::get_package_by_key($package_key)
                ?? SDFO_Commercial_CPTs::get_package_any_status($package_key);

        if (!$package) {
            return ['ok' => false, 'error' => 'cpt_package_not_found'];
        }

        if (empty($package['billing']['stripe_price_id'])) {
            return ['ok' => false, 'error' => 'cpt_package_no_stripe_price'];
        }

        $profile = SDFO_Commercial_CPTs::get_default_profile_for_package($package['post_id']);

        return self::build_cpt_pricing_result($package, $profile, $authorization_code, $prospect_post_id);
    }

    private static function build_cpt_pricing_result(
        array $package,
        ?array $profile,
        string $authorization_code,
        int $prospect_post_id
    ): array {
        $stripe_price_id     = (string) ($package['billing']['stripe_price_id'] ?? '');
        $package_key         = (string) ($package['package_key'] ?? '');
        $profile_key         = $profile ? (string) ($profile['profile_key'] ?? '') : '';
        $application_fee     = $profile ? ($profile['application_fee_policy'] ?? []) : [];
        $discount_policy     = $profile ? ($profile['discount_policy']        ?? []) : [];
        $features            = $profile ? ($profile['features']               ?? []) : [];
        $provisioning_policy = $profile ? ($profile['provisioning_policy']    ?? []) : [];
        ...
        $terms_snapshot = [
            ...
            'application_fee_policy' => $application_fee,
            'discount_policy'        => $discount_policy,
            'features'               => $features,
            'provisioning_policy'    => $provisioning_policy,
        ];

        // Write snapshot meta directly (mirrors store_commercial_snapshot shape).
        update_post_meta($prospect_post_id, 'sd_package_key',                    $package_key);
        ...
        update_post_meta($prospect_post_id, 'sd_resolved_plan_label',            $package['label']);
        update_post_meta($prospect_post_id, 'sd_discount_policy_json',           wp_json_encode($discount_policy));
        update_post_meta($prospect_post_id, 'sd_application_fee_policy_json',    wp_json_encode($application_fee));
        update_post_meta($prospect_post_id, 'sd_commercial_terms_snapshot_json', wp_json_encode($terms_snapshot));
